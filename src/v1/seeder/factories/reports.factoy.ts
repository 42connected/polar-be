...
export default setSeederFactory(Reports, (faker, meta: ReportsMetaInterface) => {
  const reports = new Reports();
  const imageUrlRoom = [];
  for (let i = 0; i < faker.datatype.number(4); i++) {
    imageUrlRoom.push(faker.image.imageUrl());
  }
  const mentoringLogsRandom = meta.mentoringLogsMeta[faker.datatype.number(meta.mentoringLogsMeta.length - 1)];
