...
export default setSeederFactory(
  KeywordCategories,
  (faker, meta: KeywordCategoriesMetaInterface) => {
    const keywordCategories = new KeywordCategories();

    keywordCategories.keywordId =
      meta.keywordsMeta[faker.datatype.number(meta.keywordsMeta.length - 1)].id;
    keywordCategories.categoryId =
      meta.categoriesMeta[
        faker.datatype.number(meta.categoriesMeta.length - 1)
      ].id;
